...
export function CameraView(props: CameraViewProps) {
  const {
    videoRef, placeholderRef, cameraState, cameras, currentCamera, menuOpen, historyCount,
    latestThumbnailUrl, captureBusy, flash, onToggleMenu, onCloseMenu, onSelectCamera,
    onCapture, onQuickSwap, onOpenHistory,
  } = props;
  const switching = cameraState.tag === "switching";
  const frontFacing = shouldMirrorPreview(currentCamera?.facing ?? "unknown");

  return (
    <section class="camera-stage" aria-label="カメラ画面" onKeyDown={(event) => {
      ...
      <video
        ...
        aria-hidden="true"
      />
      <canvas ref={placeholderRef} class={`switch-placeholder${switching ? " is-visible" : ""}`} aria-hidden="true" />
      {switching && <div class="switch-progress"><span class="spinner" aria-hidden="true" /><span class="sr-only">カメラを切り替えています</span></div>}
      ...
      <header class="camera-topbar">
        ...
        <p class="camera-active material"><span aria-hidden="true" />カメラ使用中</p>
      </header>
      ...
      <footer class="camera-controls">
        ...
        >
          {latestThumbnailUrl === null
            ? <CameraIcon />
            : <img src={latestThumbnailUrl} alt="" />}
          {historyCount > 0 && <span class="count-badge" aria-hidden="true">{historyCount > 99 ? "99+" : historyCount}</span>}
        </button>
        ...
        >
          {captureBusy ? <span class="spinner spinner--dark" aria-hidden="true" /> : <span class="shutter-center" />}
        </button>

        {cameras.length > 1
          ? (
            <button class="round-control material" type="button" aria-label="別のカメラへすばやく切り替え" onClick={onQuickSwap} disabled={switching}>
              <SwapIcon />
            </button>
          )
