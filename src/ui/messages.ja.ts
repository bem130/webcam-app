...
export function cameraErrorMessage(error: CameraError): string {
  switch (error.tag) {
    ...
    case "noCamera":
      return "利用できるカメラが見つかりません。接続後に再試行してください。";
    case "cameraUnavailable":
      return "カメラを開始できません。他のアプリが使用中の可能性があります。";
    case "constraintsUnsatisfied":
      return "選択したカメラを指定した条件で開始できません。";
    case "streamEnded":
      return "カメラとの接続が終了しました。";
    case "unknown":
...
export function captureErrorMessage(error: CaptureError): string {
  switch (error.tag) {
    case "frameNotReady":
      return "映像の準備が完了していません。少し待って再撮影してください。";
...
export function clipboardErrorMessage(error: ClipboardError): string {
  switch (error.tag) {
    ...
    case "notAllowed":
      return "Clipboardへの書込みが許可されませんでした。履歴から再コピーできます。";
    case "unsupportedMime":
      return "このブラウザはPNGのClipboardコピーに対応していません。";
    case "writeFailed":
      return "撮影しましたが、Clipboardへコピーできませんでした。";
    default:
      return assertNever(error);
  }
}
