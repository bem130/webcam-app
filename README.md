...
camera streamは操作が10秒ない場合、またはdocumentがbackgroundになった場合に全trackを`stop()`して解放します。idle停止時は全画面screensaverを表示し、最初のtap、key、wheelをcamera再開だけに使うため、背後のshutterは発火しません。復帰後はkeyboard focusをshutterへ戻します。backgroundからは画面へ戻っただけでは自動再取得せず、「カメラを再開」の明示操作で直前のcameraを要求します。撮影中はcamera sourceが必要な範囲だけidle停止を延期し、Clipboardやthumbnailの完了待ちではcameraを保持しません。
...
video-frameの比較診断では通常URLがWorker経路、`?videoFramePipeline=canvas`付きURLがmain-thread baselineです。選択は保存されず、実際の経路は履歴detailsの「動画処理経路」とhandoff timingで確認できます。
