...
export function chooseQuickSwapTarget(
  ...
  if (previous.tag === "some" && previous.value !== currentValue(current) && hasCamera(cameras, previous.value)) {
    return previous;
  }

  const currentIndex = current.tag === "some"
    ? cameras.findIndex((camera) => camera.id === current.value)
    : -1;
